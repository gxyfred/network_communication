...
int main ()
{
	...
	int count=1;  //count will help us know how many lines this AND server has processed
	memset(x,0,sizeof(x));
	memset(y,0,sizeof(y));
	memset(z,0,sizeof(z));
	...
	int d=0;
	int flag;  //flag and r help us get the result string
	int r;

	//Configure the UDP connection with edge server in the next part, and some of this function is copied from the allowed youtube video
